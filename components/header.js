  ...
  return (
    <>
      <header
        id="header"
        className={` ${
          !existHero || !heroVisible || openModal
            ? 'bg-body duration-[200ms]  '
            : 'bg-gradient-to-b from-[#000000bf] to-transparent duration-[300ms]'
        }
      ...
      >
        ...
        <div
          className={`order order-1 md2:absolute md2:inset-x-0 md2:w-max md2:top-[14px] vw:top-[.7291vw] md2:m-auto md2:h-max select-none md2:order-2`}
        >
          <Link href="/" passHref>
            <a
              onClick={() => setOpenModal(false)}
              className="block cursor-pointer w-[170px] vw:w-[10vw]"
            >
              <Image
                src="/images/logo.png"
                width={170}
          ...
          {reservationsButton && (
            <Link href={reservationsButton?.link?.url}>
              <a onClick={() => setOpenModal(false)}>
                <p
                  className={`font-[390] text-[#E7E3DA] text-base leading-[100%] px-3 py-2 border border-[#E7E3DA] transition-colors vw:text-[.9375vw] vw:leading-[1.388] tracking-[.05em] uppercase hover:text-body hover:bg-[#E7E3DA]`}
                >
                              ...
                              [...locations].reverse().map((location, i) => {
                                ...
                                return (
                                  <div key={_id}>
                                    {!comming_soon && (
                                      <Link
                                        href={`/menus/${current}?menu=dinner-menu`}
                                        passHref
                                      >
                                        <a
                                          onClick={() => setOpenModal(false)}
